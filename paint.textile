...
They are meant for easily painting your own designs on the cars and contain seperate layers for details, shades, design and other stuff. For guidance they also include the wireframe for exact positioning.

This templates exist to get you a quick introduction into carpainting, still they don't cover all work required and are only meant as a starting point. So it's your task to use them efficiently for the best results possible. They feature:
...
F1-2005 was our last mod, that used shadows, which were manually painted by the artist. Since then we developed some automated ways to render shadows on a seperate texture and compose it into the car texture. The results were amazing and we kept using this method, I think you’ll agree why.
...
Note, the chrome surface of McLaren utilizes the same methods, but is worth its "own complete article":http://devblog.ctdp.net/2009/03/mclaren-polished/, so we don’t cover it here.  
