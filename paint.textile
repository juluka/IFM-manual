...
Note, the chrome surface of McLaren utilizes the same methods, but is worth a hole unique article, so we don’t cover it here. Additionally to the alpha-strength layer, we often use an alpha-black layer, that covers all parts on the texture, that are not supposed to be reflective.
